...
**级别**: 审核成功
**描述**: 用户注销了计算机账户。此事件在登录会话被销毁时生成，通常与 4624（登录成功）配对用于分析完整的用户会话周期。

**重要字段**:

| 字段 | 说明 | 应急场景 |
|------|------|----------|
| TargetUserName | 注销的账户名 | 确认哪个用户退出 |
| TargetDomainName | 账户所在域 | 域内/本地账户 |
| TargetLogonId | 登录会话 ID | 关联对应的 4624 事件 |
| LogonType | 登录类型 | 判断会话类型（交互/网络/RDP） |

**应急场景**:
- **异常短时会话**: 4624 后几秒钟内出现 4634 → 可能是自动化攻击工具快速尝试
- **会话时长分析**: 4624 到 4634 的时间差 = 用户会话持续时间，异常长时间会话需关注
- **离线检测**: 工作站锁定后长时间无 4634 → 用户可能离开未注销
- **关联分析**: LogonType=10 (RDP) 的 4634 表示远程会话结束，结合 4624 可追踪完整 RDP 活动

**会话分析示例**:
```
4624 (LogonType=10, 09:00:00) → 用户通过 RDP 登录
4688 (cmd.exe, 09:05:00) → 执行命令
4634 (LogonId=相同, 09:30:00) → 用户注销，会话持续 30 分钟
```
