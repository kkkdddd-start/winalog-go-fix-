...
func GetAlertRules() []*rules.AlertRule {
	return []*rules.AlertRule{
		...
		{
			Name:        "sysmon-timestomping",
			Description: "Sysmon文件时间戳篡改检测(Event 2)",
			Enabled:     true,
			Severity:    types.SeverityHigh,
			Score:       75,
		...
		},
		{
			...
			Description: "Sysmon服务状态变化(Event 4)",
			Enabled:     true,
			Severity:    types.SeverityMedium,
			Score:       40,
			MitreAttack: "T1562.001",
		...
		},
		{
			...
			Description: "Sysmon关键进程终止检测(Event 5)",
			Enabled:     true,
			Severity:    types.SeverityHigh,
			Score:       70,
			MitreAttack: "T1562.001",
			...
			Message: "Critical process terminated: {{.Image}}",
			Tags:    []string{"defense-evasion", "process"},
		},
		{
			Name:        "sysmon-raw-disk-access",
			Description: "Sysmon原始磁盘读取-凭据窃取或勒索软件(Event 9)",
			Enabled:     true,
			Severity:    types.SeverityCritical,
			Score:       90,
			MitreAttack: "T1003.001",
		...
		},
		{
			...
			Description: "Sysmon注册表键重命名-持久化后门(Event 14)",
			Enabled:     true,
			Severity:    types.SeverityMedium,
			Score:       55,
			MitreAttack: "T1112",
		...
		{
			Name:        "sysmon-ads-file-stream",
			Description: "Sysmon ADS备用数据流-恶意软件投放(Event 15)",
			Enabled:     true,
			Severity:    types.SeverityHigh,
			Score:       80,
			MitreAttack: "T1564.004",
			Filter: &rules.Filter{
				EventIDs: []int32{15},
				Levels:   []string{"Info"},
							LogNames: []string{"Microsoft-Windows-Sysmon/Operational"},
			...
			Message: "Alternate Data Stream with executable content detected",
			Tags:    []string{"defense-evasion", "malware"},
		},
		{
			Name:        "sysmon-config-state-changed",
			Description: "Sysmon配置状态变化(Event 16)",
			Enabled:     true,
			Severity:    types.SeverityMedium,
			Score:       45,
			MitreAttack: "T1562.001",
		...
		},
		{
			...
			Description: "Sysmon驱动程序加载-恶意驱动检测(Event 6)",
			Enabled:     true,
			Severity:    types.SeverityHigh,
			Score:       70,
			...
			Conditions: &rules.Conditions{
				Any: []*rules.Condition{
					...
					{Field: "hashes", Operator: "==", Value: ""},
				},
			},
			Message: "Suspicious driver loaded from unusual path: {{.ImagePath}}",
			Tags:    []string{"persistence", "driver", "privilege-escalation"},
		},
		{
			Name:        "sysmon-registry-object-create-delete",
			Description: "Sysmon注册表键创建/删除检测(Event 12)",
			Enabled:     true,
			Severity:    types.SeverityMedium,
			Score:       50,
			MitreAttack: "T1112",
		...
		{
			Name:        "sysmon-wmi-consumer-creation",
			Description: "Sysmon WMI消费者创建-WMI持久化中间环节(Event 20)",
			Enabled:     true,
			Severity:    types.SeverityHigh,
			Score:       75,
			MitreAttack: "T1546.003",
		...
		{
			Name:        "sysmon-wmi-filter-creation",
			Description: "Sysmon WMI过滤器创建-WMI持久化(Event 19)",
			Enabled:     true,
			Severity:    types.SeverityHigh,
			Score:       75,
			MitreAttack: "T1546.003",
		...
		{
			Name:        "sysmon-wmi-consumer-binding",
			Description: "Sysmon WMI消费者绑定-持久化激活(Event 21)",
			Enabled:     true,
			Severity:    types.SeverityCritical,
			Score:       90,
			MitreAttack: "T1546.003",
		...
		{
			Name:           "sysmon-wmi-persistence-chain",
			Description:    "Sysmon WMI持久化完整链(Event 19+21关联)",
			Enabled:        true,
			Severity:       types.SeverityCritical,
			Score:          95,
			MitreAttack:    "T1546.003",
		...
		{
			Name:        "powershell-module-loading",
			Description: "PowerShell模块加载-可疑模块检测(4103)",
			Enabled:     true,
			Severity:    types.SeverityHigh,
			Score:       80,
			MitreAttack: "T1059.001",
			...
			Conditions: &rules.Conditions{
				Any: []*rules.Condition{
					{Field: "message", Operator: "regex", Value: "(?i)(mimikatz|powerview|powerup|bloodhound|sharphound|invoke-kerberoast|seatbelt|rubeus|adfind|invoke-mimikatz)"},
				},
			},
			Message: "Suspicious PowerShell module loaded - potential attack tool",
			Tags:    []string{"powershell", "execution", "credential-access"},
		},
	}
}
...
var ruleDetails = map[string]RuleDetail{
	...
	"sysmon-timestomping": {
		RuleID:         "T1070.006",
		Explanation:    "文件时间戳篡改(Timestomping)是攻击者掩盖恶意活动的反取证技术。通过修改文件的创建/修改时间使其看起来像合法文件。",
		Recommendation: "1. 审查文件时间戳异常\n2. 交叉比对NTFS元数据\n3. 检查$MFT和$UsnJrnl\n4. 使用时间线分析工具",
		RealCase:       "攻击者在植入后门后修改文件时间戳，使其与系统文件一致以避免被发现。",
	},
	"sysmon-service-state-changed": {
		RuleID:         "T1562.001",
		Explanation:    "Sysmon服务状态变化可能是攻击者试图禁用安全监控。攻击者常在入侵后停止Sysmon服务。",
		Recommendation: "1. 确保Sysmon服务被保护\n2. 监控服务状态变更\n3. 使用EDR重叠监控\n4. 配置服务恢复选项",
		RealCase:       "攻击者获取管理员权限后停止Sysmon服务以覆盖攻击痕迹。",
	},
	"sysmon-process-terminated": {
		RuleID:         "T1562.001",
		Explanation:    "关键系统进程(Lsass/winlogon/csrss/services)被终止是严重的安全事件。Lsass终止会导致系统重启且可能涉及凭据窃取。",
		Recommendation: "1. 保护关键进程不被终止\n2. 启用进程保护策略\n3. 监控异常进程终止\n4. 调查终止进程的来源",
		RealCase:       "攻击者终止Lsass进程触发系统重启，强迫管理员登录并捕获凭据。",
	},
	"sysmon-raw-disk-access": {
		RuleID:         "T1003.001",
		Explanation:    "原始磁盘读取绕过了文件系统访问控制。攻击者使用此技术读取注册表Hive、NTDS.dit或加密磁盘。",
		Recommendation: "1. 审计原始磁盘访问权限\n2. 监控扇区级读取操作\n3. 部署磁盘完整性保护\n4. 限制进程对PhysicalDrive的访问",
		RealCase:       "勒索软件通过直接读写原始磁盘扇区来加密文件，绕过文件系统监控。",
	},
	"sysmon-registry-key-rename": {
		RuleID:         "T1112",
		Explanation:    "注册表键重命名可能用于隐藏持久化机制。攻击者通过重命名键来混淆注册表结构。",
		Recommendation: "1. 监控注册表键重命名操作\n2. 审核异常注册表变更\n3. 使用注册表配置基线\n4. 限制注册表修改权限",
		RealCase:       "攻击者重命名注册表键以隐藏WMI持久化的启动配置。",
	},
	"sysmon-ads-file-stream": {
		RuleID:         "T1564.004",
		Explanation:    "NTFS备用数据流(ADS)可用于隐藏恶意代码。攻击者将可执行文件隐藏在ADS中，在文件系统中不可见。",
		Recommendation: "1. 使用dir /r扫描ADS\n2. 监控ADS文件创建\n3. 部署流检测工具\n4. 限制可执行流执行",
		RealCase:       "攻击者将后门程序存储在svchost.exe的备用数据流中，通过wmic调用执行。",
	},
	"sysmon-config-state-changed": {
		RuleID:         "T1562.001",
		Explanation:    "Sysmon配置状态变化表示监控规则被修改。攻击者可能修改Sysmon配置以关闭特定检测。",
		Recommendation: "1. 监控Sysmon配置变更\n2. 保护配置文件权限\n3. 使用注册表HKCU监控\n4. 配置变更告警",
		RealCase:       "攻击者修改Sysmon配置移除进程创建监控规则以隐藏后门进程。",
	},
	"sysmon-driver-loaded": {
		RuleID:         "T1547.006",
		Explanation:    "驱动程序加载是内核级别操作。攻击者常从临时目录或用户目录加载恶意驱动实现持久化或提权。",
		Recommendation: "1. 审核驱动程序签名\n2. 监控非标准路径驱动加载\n3. 启用驱动签名强制\n4. 限制驱动加载权限",
		RealCase:       "攻击者将恶意驱动放在Temp目录并加载，实现内核级权限获取。",
	},
	"sysmon-registry-object-create-delete": {
		RuleID:         "T1112",
		Explanation:    "注册表对象的创建和删除是持久化攻击的常见手段。攻击者通过创建注册表键来配置自启动或服务。",
		Recommendation: "1. 监控注册表键变更\n2. 审核新创建的注册表键\n3. 使用注册表配置基线\n4. 限制注册表修改权限",
		RealCase:       "攻击者创建新的注册表键实现服务型持久化，在系统启动时自动运行恶意程序。",
	},
	"sysmon-wmi-consumer-creation": {
		RuleID:         "T1546.003",
		Explanation:    "WMI Event Consumer创建是WMI持久化链的关键环节。Consumer定义触发后执行的具体动作(如命令执行)。",
		Recommendation: "1. 立即调查Consumer内容\n2. 检查是否存在对应Filter\n3. 删除恶意Consumer\n4. 审计WMI命名空间",
		RealCase:       "攻击者创建CommandLineEventConsumer绑定系统启动事件，定时执行PowerShell下载器。",
	},
	"sysmon-wmi-filter-creation": {
		RuleID:         "T1546.003",
		Explanation:    "WMI Event Filter创建是WMI持久化的第一步。攻击者创建过滤器定义触发条件(如系统启动)，随后绑定消费者执行恶意代码。",
		Recommendation: "1. 监控WMI Filter创建\n2. 审计WMI命名空间变更\n3. 限制WMI提供者数量\n4. 使用Autoruns扫描WMI",
		RealCase:       "APT组织创建WMI Event Filter绑定系统启动事件，实现长期无文件持久化。",
	},
	"sysmon-wmi-consumer-binding": {
		RuleID:         "T1546.003",
		Explanation:    "WMI Event Consumer绑定完成持久化链。Consumer定义了触发后的具体动作(如启动脚本)。Filter+Consumer构成完整的WMI持久化机制。",
		Recommendation: "1. 立即调查Consumer内容\n2. 删除恶意WMI绑定\n3. 恢复受损系统\n4. 扫描其他主机是否受影响",
		RealCase:       "攻击者创建CommandLineEventConsumer绑定系统启动Filter，每隔5分钟执行一次PowerShell后门。",
	},
	"sysmon-wmi-persistence-chain": {
		RuleID:         "T1546.003",
		Explanation:    "Sysmon Event 19和21的关联检测可识别完整的WMI持久化攻击链。Filter定义触发条件，Consumer定义恶意动作，两者必须同时存在才能实现持久化。",
		Recommendation: "1. 紧急排查所有WMI绑定\n2. 删除所有非授权WMI对象\n3. 审计所有主机WMI配置\n4. 部署WMI使用白名单",
		RealCase:       "APT组织在域内多台服务器同时创建WMI Filter和CommandLineEventConsumer对象，实现分布式持久化。",
	},
	"powershell-module-loading": {
		RuleID:         "T1059.001",
		Explanation:    "PowerShell模块加载日志(Event 4103)记录了所有加载的PS模块。攻击者常用的渗透测试工具(Mimikatz/PowerView/BloodHound等)可通过模块名检测。",
		Recommendation: "1. 启用PowerShell模块日志\n2. 审查加载的模块列表\n3. 阻止未授权脚本执行\n4. 使用受限语言模式",
		RealCase:       "攻击者在域内主机加载Invoke-Mimikatz模块窃取凭据，4103日志记录了完整的模块加载路径和时间。",
	},
}
