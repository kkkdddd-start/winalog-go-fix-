...
function SystemInfo() {
  const { t } = useI18n()
  const [activeTab, setActiveTab] = useState<'system' | 'processes' | 'network' | 'env' | 'dlls' | 'drivers' | 'users' | 'registry' | 'startup' | 'tasks' | 'services' | 'runkeys' | 'userinit' | 'patches' | 'software'>('system')
  ...
  useEffect(() => {
    ...
    if (enabledModules.patches && patches.length === 0) {
      fetchPatches()
    }
    if (enabledModules.software && software.length === 0) {
      fetchSoftware()
    }
  }, [enabledModules])
  ...
  const fetchPatches = () => {
    ...
    systemAPI.getPatches()
      ...
      .catch((err: any) => {
        const msg = err.response?.status === 404
          ? '补丁信息不可用（仅支持 Windows）'
          : (err.message || '获取补丁信息失败')
        setModuleErrors(m => ({ ...m, patches: msg }))
        setLoading(false)
      })
  }
  ...
  const fetchSoftware = () => {
    ...
    systemAPI.getSoftware()
      ...
      .catch((err: any) => {
        const msg = err.response?.status === 404
          ? '软件列表不可用（仅支持 Windows）'
          : (err.message || '获取软件列表失败')
        setModuleErrors(m => ({ ...m, software: msg }))
        setLoading(false)
      })
  }

  const handleTabChange = (tab: 'system' | 'processes' | 'network' | 'env' | 'dlls' | 'drivers' | 'users' | 'registry' | 'startup' | 'tasks' | 'services' | 'runkeys' | 'userinit' | 'patches' | 'software') => {
    setActiveTab(tab)
    ...
    if (tab === 'patches' && enabledModules.patches) fetchPatches()
  ...
  const handleExport = (type: string) => {
    switch (type) {
      ...
      case 'env':
        ...
        break
      case 'patches':
        exportToCSV(patches, 'patches', ['kb_id', 'description', 'installed_on', 'installed_by'])
        break
      case 'software':
        exportToCSV(software, 'software', ['name', 'version', 'publisher', 'install_date', 'estimated_size_mb', 'architecture'])
        break
    }
  }
            ...
            <div className="export-dropdown">
              ...
              {showExportMenu && (
                <div className="export-menu">
                  ...
                  {activeTab === 'patches' && (
                    <>
                      <button onClick={() => handleExport('patches')}>前端导出（已加载数据）</button>
                      <button onClick={() => handleExportFromBackend('patches')}>后端导出（完整数据）</button>
                    </>
                  )}
                  {activeTab === 'software' && (
                    <>
                      <button onClick={() => handleExport('software')}>前端导出（已加载数据）</button>
                      <button onClick={() => handleExportFromBackend('software')}>后端导出（完整数据）</button>
                    </>
                  )}
                </div>
              )}
...
<button
        ...
        >
          <span className="tab-label">{t('systemInfo.patches') || '系统补丁'} ({patches.length || '...'})</span>
          <label className="module-toggle" onClick={e => e.stopPropagation()}>
            <input
              type="checkbox"
              checked={enabledModules.patches}
              onChange={() => setEnabledModules(m => ({...m, patches: !m.patches}))}
            />
            <span className="toggle-slider"></span>
          </label>
        </button>
        ...
        >
          <span className="tab-label">{t('systemInfo.software') || '软件列表'} ({software.length || '...'})</span>
          <label className="module-toggle" onClick={e => e.stopPropagation()}>
            <input
              type="checkbox"
              checked={enabledModules.software}
              onChange={() => setEnabledModules(m => ({...m, software: !m.software}))}
            />
            <span className="toggle-slider"></span>
          </label>
        </button>
      ...
      {activeTab === 'patches' && (
        <div className="tab-content">
          ...
          {patches.length === 0 && !loading && (
            <div className="empty-state">{t('systemInfo.noPatchData') || '暂无补丁信息'}</div>
          )}
        </div>
      )}
      ...
      {activeTab === 'software' && (
        <div className="tab-content">
          ...
          {software.length === 0 && !loading && (
            <div className="empty-state">{t('systemInfo.noSoftwareData') || '暂无软件信息'}</div>
          )}
        </div>
      )}
