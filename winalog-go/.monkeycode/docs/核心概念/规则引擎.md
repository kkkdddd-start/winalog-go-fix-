...
`None` 是反向匹配逻辑：事件命中 None 中**任一条**件即被排除，不触发告警。

```
事件 → Filter 匹配 → Any/All 条件匹配 → None 条件检查
                                              ↓
                                    命中 None？→ 是 → 排除事件（白名单）
                                              ↓
                                              否 → 触发告警
...
**适用场景**：
- 排除正常软件/系统操作（如 Defender 驱动、打印机服务）
- 排除机器账户行为（如 `COMPUTER$@DOMAIN` 格式的 Kerberos 请求）
- 排除已知误报进程/路径

**风险评估原则**：
| 排除条件类型 | 漏报风险 | 示例 |
|-------------|---------|------|
| 系统驱动/服务路径 | 极低 | `C:\Windows\System32\drivers\WdFilter.sys` |
| 签名软件独有行为模式 | 低 | `TitanAgent\tmp\ntds` VSS 路径 |
| 机器账户格式特征 | 零 | `$@` 后缀（密码 120 位随机值无破解可能） |
| 硬编码主机名/IP | 不可用 | 环境差异导致漏报 |

**与 Filter 的区别**：
- `Filter`: 正向筛选，"只处理这些事件"
- `Conditions.All`: 正向匹配，"事件必须满足所有条件"
- `Conditions.None`: 反向排除，"排除满足任一条件的事件"
