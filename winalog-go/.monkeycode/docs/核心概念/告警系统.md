...
### 规则层面的误报说明（`false_positive_notes`）

每条告警由规则触发，规则本身具有 `false_positive_notes` 字段（定义在 `internal/rules/builtin/definitions.go` 的 `RuleDetail` 结构体中）：

- **作用**：记录该规则的白名单条件、不可修复的已知误报及原因、研判提示
- **数据流**：`RuleDetail.FalsePositiveNotes` → `GetFalsePositiveNotes()` → API `/api/rules` 的 `RuleInfo` → 前端 Rules.tsx 详情弹窗
- **与 Alert 的区别**：`Alert.FalsePositive` 是单条告警级别的误报标记，`RuleDetail.FalsePositiveNotes` 是规则级别的白名单说明
...
用户可见的「白名单」概念（`winalog whitelist` CLI 命令和 Web UI `/suppress` 页面）在引擎层面映射为 `SuppressCache` 的抑制规则：

- `winalog whitelist add` → 创建 `SuppressRule` 条目 → 加载到 `SuppressCache`
- 白名单/抑制条目可指定过滤条件：事件 ID、来源、用户、计算机、规则名
- 作用范围：`global`（全局，抑制所有匹配事件）或 `rule`（仅抑制特定规则）

这是告警削减的三层体系：
1. **规则内置白名单**（`Conditions.None`）：在评估阶段排除已知正常模式
2. **用户白名单/抑制**（`SuppressCache`）：用户手动配置的告警抑制
3. **告警标记**（`Alert.FalsePositive`）：事后标记单条告警为误报
